...
contract Migrator is Ownable {
    ...
    function migrateTokens(bytes[] memory tokenDataList) public onlyOwner {
        for (uint256 i = 0; i < tokenDataList.length; i++) {
            lazbubu.migrateToken(tokenDataList[i]);
        }
    }

    function migrateAdventures(bytes[] memory adventureData) public onlyOwner {
    ...
    function migrateMemories(bytes[] memory memoryData) public onlyOwner {
